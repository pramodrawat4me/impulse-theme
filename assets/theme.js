    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function({srcElement}) {
        ...
        if(document.querySelector('.rte')){
            if(glass_nonglass_label[variant.id]){
              document.querySelector('.rte span').innerHTML = 'STYLE# ' + glass_nonglass_label[variant.id];
            }

        }
        if(document.querySelector('.rte-des')){
            if(glass_des[variant.id]){
              document.querySelector('.rte-des').innerHTML = glass_des[variant.id];
            }
      ...
      _updatePrice: function(variant) {
        var splitName = variant.name.split(' - ');
        var variantOptions = splitName[1].replace(' / ', ' with ');
        variantOptions = variantOptions.replace(' / ', ' , ');
        var lastIndexOf2 = variantOptions.lastIndexOf('2');
        variantOptions = variantOptions.substring(0, lastIndexOf2) + '' + variantOptions.substring(lastIndexOf2);
        console.log(variantOptions);
        if(document.querySelector('.products-variants-title')){
          document.querySelector('.products-variants-title').textContent = variantOptions;
        }
